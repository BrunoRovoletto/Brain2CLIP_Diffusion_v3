...
class CLIPImageEncoder():
    def __init__(
        ...
        self.image_encoder = image_encoder.from_pretrained("openai/clip-vit-large-patch14")
        self.image_processor = feature_extractor.from_pretrained("openai/clip-vit-large-patch14")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(input_image, PIL.Image.Image):
            ...
            raise ValueError(f"`input_image` has to be of type `PIL` or `list` but is {type(input_image)}")
...
class StableDiffusionFromLatents(DiffusionPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
        image_encoder: CLIPModel,
